...
import { ShowcaseService } from '../showcases/showcase.service';
...
export class EmployeeStatusGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.method === 'POST') {
      ...
      let showcaseId = request.params.id
      console.log('authorization showcase id found', showcaseId)
      if (!showcaseId) {
        return false; // showcase not found, denying access
      }
      let showcase = await this.showcaseService.findOne(showcaseId)
      organization = await this.organizationService.findRecord(showcase.organization.id)
    }
