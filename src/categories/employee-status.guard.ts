...
export class EmployeeStatusGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.method === 'POST') {
      ...
      console.log('authorization category id found', categoryId)
      if (!categoryId) {
        return false; // category not found, denying access
      }
      let category = await this.categoryService.findOne(categoryId)
      organization = await this.organizationService.findRecord(category.organization.id)
    }
