...
export class EmployeeStatusGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.method === 'POST') {
      ...
      console.log('authorization file id found', fileId)
      if (!fileId) {
        return false; // file not found, denying access
      }
      let file = await this.fileService.findOne(fileId)
      organization = await this.organizationService.findRecord(file.organization.id)
    }
