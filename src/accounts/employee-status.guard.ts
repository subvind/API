...
export class EmployeeStatusGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.method === 'POST') {
      ...
      console.log('authorization account id found', accountId)
      if (!accountId) {
        return false; // account not found, denying access
      }
      let account = await this.accountService.findOne(accountId)
      organization = await this.organizationService.findRecord(account.organization.id)
    }
