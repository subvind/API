...
export class EmployeeStatusGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.method === 'POST') {
      ...
      console.log('authorization playlist id found', playlistId)
      if (!playlistId) {
        return false; // playlist not found, denying access
      }
      let playlist = await this.playlistService.findOne(playlistId)
      organization = await this.organizationService.findRecord(playlist.organization.id)
    }
