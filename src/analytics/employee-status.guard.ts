...
export class EmployeeStatusGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.method === 'POST') {
      ...
      let analyticId = request.params.id
      console.log('authorization analytic id found', analyticId)
      if (!analyticId) {
        return false; // analytic not found, denying access
      }
      let analytic = await this.analyticService.findOne(analyticId)
      organization = await this.organizationService.findRecord(analytic.organization.id)
    }
