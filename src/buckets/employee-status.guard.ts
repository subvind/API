...
export class EmployeeStatusGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.method === 'POST') {
      ...
      let bucketId = request.params.id
      console.log('authorization bucket id found', bucketId)
      if (!bucketId) {
        return false; // bucket not found, denying access
      }
      let bucket = await this.bucketService.findOne(bucketId)
      organization = await this.organizationService.findRecord(bucket.organization.id)
    }
