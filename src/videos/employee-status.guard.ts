...
export class EmployeeStatusGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.method === 'POST') {
      ...
      console.log('authorization video id found', videoId)
      if (!videoId) {
        return false; // video not found, denying access
      }
      let video = await this.videoService.findOne(videoId)
      organization = await this.organizationService.findRecord(video.organization.id)
    }
